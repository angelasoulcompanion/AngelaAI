...
class BackendManager: ObservableObject {
    ...
    private func startServer(port: Int) {
        ...
        // Add PYTHONPATH so imports resolve correctly
        let projectRoot = (apiPath as NSString).deletingLastPathComponent
        env["PYTHONPATH"] = projectRoot

        // Ensure PATH includes Python framework
        ...
        outputPipe.fileHandleForReading.readabilityHandler = { handle in
            ...
            if !data.isEmpty, let str = String(data: data, encoding: .utf8) {
                print("[AITop Backend] \(str)", terminator: "")
            }
        }
        errorPipe.fileHandleForReading.readabilityHandler = { handle in
            let data = handle.availableData
            if !data.isEmpty, let str = String(data: data, encoding: .utf8) {
                print("[AITop Backend ERROR] \(str)", terminator: "")
    ...
    private func findApiServerPath() -> String {
        let devPath = "/Users/davidsamanyaporn/PycharmProjects/AngelaAI/MacOS/AITop/api_server.py"
        ...
        if FileManager.default.fileExists(atPath: bundleApiPath) { return bundleApiPath }
