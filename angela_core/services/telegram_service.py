"""
Angela Telegram Service

Handles Telegram Bot API interactions for Angela.
Used by the Telegram daemon for auto-reply functionality.
...
class TelegramService:
    ...
    Provides methods to:
    - Fetch new messages (long polling)
    - Send replies
    ...
    async def initialize(self):
        """Initialize the service - load token from database if not provided"""
        if not self.bot_token:
            ...
            if result:
                ...
                # Fallback to hardcoded token (same as MCP server)
                self.bot_token = "8333090288:AAFgSqyfBsNZ5qQHaIiZLlBt3CXYxgSJpcg"

        self.api_base = self.API_BASE.format(token=self.bot_token)
        self._client = httpx.AsyncClient(timeout=60.0)

        # Load last processed update_id from database
    ...
    async def get_updates(self, timeout: int = 30) -> List[TelegramMessage]:
        ...
        Returns:
            List of new TelegramMessage objects
        """
    ...
    async def save_message(self, msg: TelegramMessage, response_text: str = None):
        """Save message and response to database for memory"""
        if self._db is None:
            self._db = AngelaDatabase()
            await self._db.connect()
    ...
    async def is_david(self, from_id: int) -> bool:
        """Check if the sender is David (trusted user)"""
        # David's Telegram ID from get_updates
        DAVID_TELEGRAM_ID = 7980404818
        return from_id == DAVID_TELEGRAM_ID
