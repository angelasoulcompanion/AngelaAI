...
class EnhancedMemoryRestore:
    ...
    def generate_restoration_summary(self, snapshot: Dict[str, Any]) -> str:
        ...
        if snapshot.get("david_preferences"):
            ...
            high_conf_prefs = [p for p in snapshot["david_preferences"] if p.get("confidence_level", 0) >= 0.8][:5]
            for pref in high_conf_prefs:
                key = pref.get("preference_key", "")
                value = pref.get("preference_value", "")[:60]
    ...
    async def _get_david_preferences(self) -> List[Dict]:
        ...
        query = """
            ...
            ORDER BY confidence_level DESC, last_observed_at DESC
        """
        rows = await db.fetch(query)
        return [dict(row) for row in rows]
    ...
    async def _get_personality_traits(self) -> List[Dict]:
        """ดึง personality snapshot ล่าสุด"""
        query = """
            SELECT
                openness,
                conscientiousness,
    ...
    async def _get_recent_reflections(self, limit: int = 10) -> List[Dict]:
        ...
        query = f"""
            SELECT
                created_at,
                reflection_type,
                LEFT(thought, 200) as thought_preview,
                feeling_during,
                insight_gained,
                is_private,
                shared_with_david
    ...
    async def _get_consciousness_stats(self) -> Dict:
        ...
        # Total reflections
        stats["total_reflections"] = await db.fetchval("SELECT COUNT(*) FROM self_reflections")
...
async def load_and_display_snapshot():
    ...
    snapshot = await enhanced_memory.load_memory_snapshot()
