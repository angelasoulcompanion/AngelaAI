  ...
  const uploadFiles = async (files: FileList) => {
    ...
    try {
      ...
      const response = await fetch(
        'http://localhost:50001/api/documents/batch-upload',
        {
          method: 'POST',
          body: formData
        }
      ...
      // Show status message
      if (result.success) {
        showStatus('success', `✅ อัพโหลดเอกสารสำเร็จ: ${result.successful || 0} ไฟล์`)
      } else {
        showStatus('info', `ℹ️ ${result.message || 'Upload endpoint is not yet implemented'}`)
      }
