...
async def get_core_memories(limit: int = Query(20, ge=1, le=100), conn=Depends(get_conn)):
    """Fetch core memories"""
...
@router.get("/dreams")
async def get_subconscious_dreams(limit: int = Query(10, ge=1, le=50), conn=Depends(get_conn)):
...
@router.get("/growth")
async def get_emotional_growth(conn=Depends(get_conn)):
...
@router.get("/summary")
async def get_subconsciousness_summary(conn=Depends(get_conn)):
