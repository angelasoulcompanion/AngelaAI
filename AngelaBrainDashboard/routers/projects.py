...
@router.get("/list")
async def get_projects(conn=Depends(get_conn)):
    """Fetch all projects"""
...
@router.get("/sessions")
async def get_recent_work_sessions(days: int = Query(7, ge=1, le=30), conn=Depends(get_conn)):
...
async def get_recent_milestones(limit: int = Query(10, ge=1, le=50), conn=Depends(get_conn)):
...
async def get_recent_project_learnings(limit: int = Query(10, ge=1, le=50), conn=Depends(get_conn)):
...
async def get_tech_stack_graph(conn=Depends(get_conn)):
