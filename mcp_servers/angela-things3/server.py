    ...
    try:
        if name == "view-inbox":
            ...
            try:
                ...
                for todo in todos:
                    ...
                    when_date = todo.get("when") or "No Scheduled Date"
        ...
        if name == "view-projects":
            ...
            try:
                ...
                for project in projects:
                    ...
                    response.append(f"\n• {title}{area_info} ({open_tasks} open tasks)")
        ...
        if name == "view-todos":
            ...
            try:
                ...
                response = ["Todos in Things3 (Today):"]
                for todo in todos:
                    title = (todo.get("title", "Untitled Todo")).strip()
                    project = todo.get("project") or ""
                    area = todo.get("area") or ""
        ...
        if name == "search-things3-todos":
            ...
            if not await asyncio.to_thread(sqlite_handler.validate_access):
                return [
                    types.TextContent(
                        type="text",
                        text="Things3 database not found. Please ensure Things3 is installed.",
                    )
                ]
