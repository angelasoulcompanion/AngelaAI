"""
David Gmail MCP Server

Send and read emails from David's Gmail account (davidsamanyaporn@gmail.com)
"""
...
from shared import DAVID_EMAIL
...
# Setup logging
logger = setup_logging("david-gmail")

# Create MCP Server
server = Server("david-gmail")
...
async def list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            name="send_email",
            description="Send an email from David (davidsamanyaporn@gmail.com)",
            inputSchema={
                "type": "object",
                "properties": {
                    "to": {
...
async def send_email(service, args: dict) -> list[TextContent]:
    ...
    message['to'] = to
    message['from'] = f"David <{DAVID_EMAIL}>"
