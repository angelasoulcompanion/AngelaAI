#!/usr/bin/env python3
"""
Google Sheets OAuth Re-authentication Script for Angela

Run this script when the Sheets token expires:
    python3 mcp_servers/angela-sheets/auth.py
"""
...
def main():
    ...
    print("=" * 50)
    print("Angela Sheets Re-Authentication")
    print("=" * 50)
    ...
    # Test the connection
    print("\n[TEST] Testing Sheets API connection...")
    try:
        service = build('sheets', 'v4', credentials=creds)
        # Create a test spreadsheet to verify access
        spreadsheet = service.spreadsheets().create(
            body={'properties': {'title': 'Angela Auth Test (delete me)'}}
        ).execute()
        sheet_id = spreadsheet['spreadsheetId']
        print(f"[SUCCESS] Connected! Created test sheet: {sheet_id}")
